...
$(function () {
    ...
    var m = L.map('map').setView([13.310919, 2.603680], 14);
    m.createPane('left')
    m.createPane('right')
    ...
    var url_to_geotiff_file = "https://restoration-hosting.storage.googleapis.com/niger.tif";
    ...
    parseGeoraster(url_to_geotiff_file).then(georaster => {
      layer = new GeoRasterLayer({
          ...
          georaster: georaster,
          resolution: 512,
          pixelValuesToColorFn: values => values[0] >= 0.55 ? "#9fbb52" : null,
      });
      layer.addTo(m);
      layer.bringToFront()
      layer.setZIndex(100);
    ...
    window.change_window = function() {
        ...
        parseGeoraster(url_to_geotiff_file).then(georaster => {
          var layer = new GeoRasterLayer({
              ...
              georaster: georaster,
              resolution: 100,
              pixelValuesToColorFn: values => values[0] >= 0.1 ? "#9fbb52" : null,
          });
          layer.addTo(m);
          layer.bringToFront()
          layer.setZIndex(100);
