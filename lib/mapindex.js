...
$(function () {
    ...
    var m = L.map('map', { zoomControl: false , loadingControl: true})
    ...
    item.on('loading', function(e) { loadingControl._showIndicator() });
    item.on('load', function(e) { loadingControl._hideIndicator() });
    item.addTo(m)
    sidebyside = L.control.sideBySide(myLayer1, [myLayer2, item])
    sidebyside.addTo(m);
